    ...
    <div className="h-screen overflow-auto flex flex-col bg-background text-foreground">
      ...
      <main className="flex-1 p-6 flex flex-col gap-6">
        ...
        <section className="rounded-xl border bg-card flex flex-col gap-4 p-4"
          ...
          >
            <FlaskConical className="h-4 w-4 shrink-0" style={{ color: "hsl(221 83% 53%)" }} />
            <span className="text-sm font-semibold flex-1" style={{ color: "hsl(221 83% 53%)" }}>Synthetic Simulation</span>
            <ChevronDown
              className="h-4 w-4 shrink-0 transition-transform duration-200 text-muted-foreground group-hover:text-foreground"
              style={{ transform: simOpen ? "rotate(0deg)" : "rotate(-90deg)" }}
            />
          </button>
          {simOpen && (
            <>
              ...
              <div className="rounded-lg border px-3 py-2.5 flex items-start gap-2.5 -mt-1"
                style={{ background: "hsl(38 92% 50% / 0.06)", borderColor: "hsl(38 92% 50% / 0.25)" }}>
                <span className="text-[11px] shrink-0 mt-px">⚠️</span>
                <p className="text-[11px] leading-relaxed" style={{ color: "hsl(38 92% 50% / 0.9)" }}>
                  This is fully synthetic — data is written directly to Elasticsearch, no real infrastructure involved.
                  For the full closed-loop experience with real container restarts, use the{" "}
                  <a
                    href="https://github.com/padmanabhan-r/QuantumState/blob/main/README.md#injecting-real-faults-recommended"
                    target="_blank"
              ...
              {/* Baseline Setup */}
              <div className="rounded-lg border border-border p-3 flex items-center gap-4" style={{ background: "hsl(222 47% 3%)" }}>
